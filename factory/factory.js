...
app.factory("services", ['$http', function ($http) {
    ...
    var obj = {};
    obj.getAlumnos = function (region, distrito, rol, cargo) {
        return $http.get(serviceBase + 'alumnos?region=' + region 
                        +'&distrito=' + distrito + '&rol=' + rol + '&cargo=' + cargo);
    ...
    obj.buscarVoto = function (idalumno, comision, distrito, instancia) {
    ...
    obj.buscarPostulado = function (idalumno, comision, distrito, instancia) {
        return $http.get(serviceBase + 'buscar_postulado?idalumno=' + idalumno + 
                    '&comision=' + comision +
                    '&distrito=' + distrito +
                    '&instancia=' + instancia
                    );
    ...
    //habilitación de voto temporal ------->
    obj.updateHabilitarVotacion = function (distrito, estado) {
        console.log(estado)
        return $http.get(serviceBase + 'update_habilitar_votacion?distrito=' + distrito + '&estado=' + estado);
    }

    obj.getVotacionActivada = function (distrito) {
        return $http.get(serviceBase + 'get_votacionactivada?distrito=' + distrito);
    }

    obj.updateHabilitarPostulacion = function (distrito, estado) {
        console.log(estado)
        return $http.get(serviceBase + 'update_habilitar_postulacion?distrito=' + distrito + '&estado=' + estado);
    }

    obj.getPostuActivada = function (distrito) {
        return $http.get(serviceBase + 'get_postu_activada?distrito=' + distrito);
    }

    // FIN habilitación de voto temporal <-------




    // habilitación de votos OK
    obj.habilitarVotacion = function (comision, distrito, instancia, vuelta) {
        return $http.get(serviceBase + 'habilitar_votacion?comision=' + comision + 
                            '&distrito=' + distrito +
                            '&instancia=' + instancia +
                            '&vuelta=' + vuelta
                            );
    ...
    obj.setCronograma = function (data) {
        return $http.post(serviceBase + 'set_cronograma', data).then(function (results) {
            return results;
        });
    }

    obj.getCronograma = function (distrito) {
        return $http.get(serviceBase + 'get_cronograma?distrito=' + distrito);
    }
    ...
    obj.getListarDistritos = function (region, distrito, cargo) {
        console.log(region)
...
app.factory('LoginService', function ($rootScope, $http, Base64, $cookieStore) {
    var isAuthenticated = false,
        ...
        mostrarNotificacion = '', comision = '', region = '', distrito = '', cargo = '';
    return {
