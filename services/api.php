<?php
	...
	private function alumnos()
	{
		...
		if (isset($this->_request['region'])) {
			...
			$cargo = (string)$this->_request['cargo'];
			...
			if ($region == '0') {
			...
			} elseif ((int)$cargo == 1 || (int)$cargo == 3 || (int)$cargo == 4 || (int)$cargo == 5) {
				...
				if (isset($this->_request['distrito'])) {
					...
					$query = "SELECT f.id, CONCAT (f.apellido, ', ', f.nombre) AS alumno, f.aceptado, f.region, f.distrito, f.comision, f.colegio, f.rol   
										FROM fichas f
...
//=============================================================
//TEMPORAL updateHabilitarVotacion <<<------------------------
//antes de actualizar a la versión
//=============================================================
	private function update_habilitar_votacion()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
		...
		$query = "UPDATE regiones SET 
				voto_habilitado =	$estado 
				WHERE distrito = $distrito";


		$r = $this->mysqli->query($query) or die($this->mysqli->error . __LINE__);
		$success = array("id" => $distrito);
		$this->response($this->json($success), 200);
	}


	private function update_habilitar_postulacion()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
	...
	private function get_votacionactivada()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
	...
	private function get_postu_activada()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
	...
	//habilitar votación definitiva
	private function habilitar_votacion()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
		...
		// 				Select $idAlumno, $idvoto, $comision, $distrito, $vuelta, $instancia
		// 				FROM DUAL
		// 				where not exists (
		// 				select 
		// 					id_alumno
		...
		$query = "INSERT INTO vueltas (comision, distrito, instancia, vuelta)
						SELECT $comision, $distrito, $instancia, $vuelta
						FROM DUAL
						WHERE NOT EXISTS (
							SELECT 
								distrito
	...
	private function listar_distritos()
	{
		...
		if (isset($this->_request['region'])) {
			...
			$distrito = (int)$this->_request[ 'distrito'];
			$cargo = (int)$this->_request['cargo'];

			if((int)$cargo == 2){
				$query = "SELECT * from regiones WHERE region=$region order by nombre";
			} elseif ((int)$cargo == 1 || (int)$cargo == 3 || (int)$cargo == 4 || (int)$cargo == 5) {
				$query = "SELECT * from regiones WHERE distrito = $distrito order by nombre";
			} 
	...
	private function buscar_voto()
	{
		...
		$query = "SELECT
				CONCAT(f.apellido, ', ', f.nombre) AS alumno, v.vuelta
				FROM
						`fichas` f
	...
	private function estadisticas()
	{
		...
		$query = "SELECT 
							fichas.id as id_alumno,
							CONCAT(fichas.apellido,', ', fichas.nombre) as alumno, 
							(SELECT count(voto) from votos 
								WHERE votos.voto=postulantes.id_alumno AND
											votos.comision= $ncomision AND
											votos.distrito=$distrito AND
											votos.vuelta=$vuelta AND
											votos.instancia= $instancia 
								) as votos
							from
	...
	private function set_cronograma()
	{
		if ($this->get_request_method() != "POST") {
			$this->response('', 406);
		}
		...
		$distrito = (int)$tmp['distrito'];
		$str = $tmp['valor'];
		$valor = utf8_decode($str);
		$valor = $str;
		$query = "UPDATE regiones SET cronograma = '$valor' WHERE distrito = $distrito";
		//para evitar problemas con el set de caracteres y acentos
		$acentos = $this->mysqli->query("SET NAMES 'utf8'");
		$r = $this->mysqli->query($query) or die($this->mysqli->error . __LINE__);
		$success = array('status' => "Success", "msg" => "Se actualizó el registro.");
	...
	private function get_cronograma()
	{
		if ($this->get_request_method() != "GET") {
			$this->response('', 406);
		}
	...
	private function notificaciones(){	
		...
		*/
		...
		$query= "SELECT 
							...
							fichas.comision,
							(SELECT count(voto) from votos 
								WHERE votos.voto=postulantes.id_alumno
								) as votos
							from
							fichas  inner join postulantes  on postulantes.id_alumno = fichas.id
                            INNER JOIN REGIONES on regiones.distrito = fichas.distrito
							where (SELECT count(voto) from votos 
								WHERE votos.voto=postulantes.id_alumno
								) > 0
							ORDER BY fichas.region, fichas.distrito, fichas.comision, votos desc";
			...
			if ($r->num_rows > 0) {
				...
				$this->outputCsv('bija.csv', $result);
